...
ShellRoot {
    ...
    IpcHandler {
        ...
        function setEnabled(code: bool) : string {
    ...
    Loader {
        ...
        Connections {
            function onSecured() {
                WidgetState.confirmLockSecure();
            }

            function onUnlocked() {
                WidgetState.confirmUnlocked();
    ...
    Connections {
        function onLockRequested() {
            ...
            lockAcquireTimer.restart();
        }

        function onLockPendingChanged() {
            if (WidgetState.lockPending)
                return ;
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            if (!WidgetState.sessionLocked && WidgetState.lockPending)
