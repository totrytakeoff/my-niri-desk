...
Item {
    ...
    function getAppIconSource(id) {
        return id === "system" ? "" : "file://" + Quickshell.env("HOME") + "/.config/quickshell/assets/apps/" + id + ".svg";
    }
    ...
    function dismissMessage(messageId) {
        WidgetState.dismissMessage(appId, messageId);
    }
        ...
        delegate: Rectangle {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Image {
                        source: getAppIconSource(root.appId)
                        visible: root.appId !== "system"
                    ...
                    Text {
                        text: {
                            var trigger = root.timeRefreshTrigger;
                            return TimeUtils.getRelativeTime(modelData.timestamp);
                        }
                        font.pixelSize: 12
                        color: theme.subtext
                    }
            ...
            DragHandler {
                ...
                onActiveChanged: {
                    if (!active) {
                        if (Math.abs(parent.x) > 60)
                            root.dismissMessage(modelData.id);
                        else
