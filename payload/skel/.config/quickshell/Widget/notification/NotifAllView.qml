...
Item {
    ...
    function update() {
        allMessages = WidgetState.getAllMessages();
    }

    function getAppIconSource(appId) {
        return appId === "system" ? "" : "file://" + Quickshell.env("HOME") + "/.config/quickshell/assets/apps/" + appId + ".svg";
    }
        ...
        delegate: Rectangle {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Image {
                        source: getAppIconSource(modelData.appId)
                        visible: modelData.appId !== "system"
                ...
                onActiveChanged: {
                    if (!active) {
                        if (Math.abs(parent.x) > 60)
                            WidgetState.dismissMessage(modelData.appId, modelData.id);
                        else
