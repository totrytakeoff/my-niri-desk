...
Singleton {
    ...
    property color glass_bar_hover: Qt.rgba(inverse_surface.red, inverse_surface.green, inverse_surface.blue, 0.3)
    ...
    property color glass_island: Qt.rgba(inverse_surface.red, inverse_surface.green, inverse_surface.blue, 0.2)
    property color glass_outline: Qt.alpha(inverse_surface, 0.24)
    // 大面积浮层使用较深的中性色玻璃，保证壁纸可感知但不干扰正文。
    // 小卡片则只叠加一层轻微亮色，避免在玻璃面板里继续堆叠厚重深色块。
    property color glass_panel: Qt.alpha(background, 0.82)
    property color glass_popup: Qt.alpha(surface_container, 0.84)
    property color glass_card_subtle: Qt.alpha(inverse_surface, 0.08)
    property color glass_card_hover: Qt.alpha(inverse_surface, 0.12)
    property color glass_outline_soft: Qt.alpha(inverse_surface, 0.18)
